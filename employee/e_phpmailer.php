...
$select="SELECT * FROM epeac_employee where email='$email'";
	...
	if(mysqli_num_rows($query)>0)
	{
		...
		try {
			...
			//Recipients
			$mail->setFrom('SENDERS EMAIL ID', 'SENDERS NAME');
		 ...
		 // $mail->addAddress('ellen@example.com');               // Name is optional
		   ...
		   // $mail->addBCC('bcc@example.com');
			...
			// Content
			$url="http://".$_SERVER["HTTP_HOST"].dirname($_SERVER["PHP_SELF"])."/e_recovery_pass.php?code=$code";
			...
			$mail->send();
			header('Location:employee_login.php?pass_error=0');
		} catch (Exception $e) {
	...
	{
		header('Location:e_forget_pass.php?pass_error=1');
	}
?>
